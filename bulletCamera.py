...
def sendFrame(number):
    ...
    if not cameraOpen:
        ...
        if number >= 0 and number < FNUM:
            ...
            return "BulletCam Id:" + str(data['cam']['id']) + " - Error: index out of range", 404
...
def setBrightness():
    ...
    return "BulletCam Id:" + str(data['cam']['id']) + " - Brightness (-64 - 64) changed: " + str(number), 200
...
def setContrast():
    ...
    return "BulletCam Id:" + str(data['cam']['id']) + " - Contrast (0 - 95) changed: " + str(number), 2000
...
def setSaturation():
    ...
    return "BulletCam Id:" + str(data['cam']['id']) + " - Saturation (0-100) changed: " + str(number), 2000
...
def setHue():
    ...
    return "BulletCam Id:" + str(data['cam']['id']) + " - Hue changed: " + str(number), 2000


@app.route("/set/exposure_auto/<int:number>")
def setExposure_auto():

    # INNO Dept: Change exposure to camera using v4l-ctl

    return "BulletCam Id:" + str(data['cam']['id']) + " - Auto exposure changed to mode (0-3): " + str(number), 2000

@app.route("/calibrate")
def calibrate():

    logging.debug('Starting calibration thread')
    # INNO Dept: Find reference totems in the image and send calibrate.json file to server 
    t = threading.Thread(name='BulletCam - send frame', target=sendFrame)
    threads.append(t)
    t.start()
